                    ...
                    <div key={je.id} className={`flex flex-col px-5 py-3 gap-1 ${esTitular ? 'bg-indigo-50/40' : ''}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => setModalJugador({ id: je.jugadorId, nombre: je.jugador.nombreCompleto, posicion: je.jugador.posicion, equipo: equipoNombre(je.jugador), ligaId: ligaId ?? undefined })} className="text-sm font-medium text-gray-900 truncate hover:text-indigo-600 hover:underline text-left">{je.jugador.nombre}</button>
                          {esCapitan && (
